...
def check_github_history(config, interval):
    ...
    pr_review_list = git_hub_plugin.get_pull_requests_reviewed()
    formatting('Issues Created', issue_list)
    formatting('PR review requested', pr_list)
    formatting('PR closed', pr_closed_list)
    formatting('PR Reviewed', pr_review_list)
    formatting('PR Reviewed', assigned_issue_list)
...
def start_syncing(config, interval):
    ...
    start_review_pull_requests_workflow(github_issues=pr_review_list, jira=jira)
