# coding: utf-8
from jirasync.utils import echo_error
from jirasync.wrappers.workflow_wrapper import (
...
def start_issue_workflow(github_issues, jira, assignee=None):
    ...
    for issue in github_issues:
        ...
        if len(tasks) > 1:
            ...
            echo_error(
                "Creating Issue task for.....{}".format(issue.data["html_url"])
            )
            create_new_issue_in_backlog(
                issue, jira=jira, issue_text=issue_text, assignee=assignee
            )
        else:
...
def start_create_pull_requests_workflow(github_issues, jira, assignee=None):
    ...
    for issue in github_issues:
        ...
        if len(tasks) > 1:
            ...
            echo_error(
                "Creating PR task for.....{}".format(issue.data["html_url"])
            )
            create_pull_request_in_current_sprint(
                issue, jira=jira, issue_text=pr_text, assignee=assignee
            )
        else:
            print ("Nothing Happened")
...
def start_review_pull_requests_workflow(github_issues, jira, assignee=None):
    ...
    for issue in github_issues:
        pr_review_text = str(
            "pr_review#" + issue.project + "#" + issue.id + "#" + assignee
        )
        ...
        if len(tasks) > 1:
            ...
            echo_error(
                "Updating PR review task for .....{}".format(
                    issue.data["html_url"]
                )
            )
            update_existing_issue_in_current_sprint(
                tasks[0], issue, jira=jira, issue_text=pr_review_text
            )
        elif len(tasks) == 0:  # Create new issue in backlog.
            echo_error(
                "Creating PR review task for .....{}".format(
                    issue.data["html_url"]
                )
            )
            create_pull_request_in_current_sprint(
                issue, jira=jira, issue_text=pr_review_text, assignee=assignee
            )
        else:
            print ("Nothing Happened")
