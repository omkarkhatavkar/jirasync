...
class GitHubPlugin(object):
    def __init__(
        self, github_username, email, interval, git_urls, github_token
    ):
    ...
    def get_github_issues_created_list(self):
        ...
        stats = (
            did.cli.main(option + self.interval)[0][0].stats[0].stats[0].stats
        )
    ...
    def get_github_issues_assigned_list(self, status="None"):
        ...
        all_stats = (
            did.cli.main(option + self.interval)[0][0].stats[0].stats[2].stats
        )
        stats = []
        for issue_stat in all_stats[:]:
            ...
            elif assignees > 1 and status == str(
                issue_stat.data["state"].encode()
            ):
                stats.append(issue_stat)
        issue_list = filter_issue_list(self.git_urls, stats)
        # print_issue_list(issue_list)
    ...
    def get_opened_pull_requests(self):
        ...
        stats = (
            did.cli.main(option + self.interval)[0][0].stats[0].stats[3].stats
        )
        pr_list = []
        for stat in stats:
    ...
    def get_pull_requests_reviewed(self):
        ...
        stats = (
            did.cli.main(option + self.interval)[0][0].stats[0].stats[5].stats
        )
    ...
    def get_pull_requests_review_in_progress(self):
        ...
        stats = (
            did.cli.main(option + self.interval)[0][0].stats[0].stats[6].stats
        )
        pr_review_list = filter_issue_list(git_urls=self.git_urls, stats=stats)
        # print_issue_list(pr_review_list)
        return pr_review_list
